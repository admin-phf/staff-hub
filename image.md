...
## Layout issues to fix in the templates

Separate from the image pipeline — these are problems in how the slides render.

### 1. Prices are getting clipped (most urgent)
In the six-up and grid layouts, the big price number is cut off at the bottom edge of
the card — `$59.95` renders as `$5⬛.95` with the dollars digit sliced in half.
The price font is too tall for the space the card gives it, so the baseline overflows.
This is the worst kind of bug on a specials sheet: it makes the price unreadable.
Fix one of three ways: shrink the price font in the dense layouts, reserve a
fixed-height price zone per card, or drop the number of cards per page.

### 2. Never print `$0.00`
Advance Co-Q10 and both Bio-Practica products show `NOW $0.00` in the grid layouts —
but the *same three products* show correctly as `BUY 2, GET 1 FREE`,
`BUY 2 FOR $145.00`, and `BUY 2 FOR $85.00` in the six-up layout. So the deal data
exists; some layout variants just aren't pulling the offer type and fall back to a
zero price. Any product resolving to `$0.00` should show its offer text instead, or
be dropped from the sheet. A zero should never reach the slide.

### 3. Collapse empty card slots
The partial-fill pages print one product plus five empty placeholder boxes, which
looks unfinished. Empty slots should collapse so the page only shows real products.

### 4. SAVE badge overlaps the price
In the dense grids the red `SAVE $X` badge sits on top of or too close to the price
text. Give the badge its own corner and keep it clear of the price zone.

### 5. Consistency clean-ups
- Product name is spelled `PlusE` in one layout and `PluSe` in another — pick one.
