(function(global){
  ...
  const els={
    referenceStrip:document.querySelector('#referenceStrip'),referenceReady:document.querySelector('#referenceReady'),referenceDot:document.querySelector('#referenceDot'),
  ...
  async function refreshReferenceStatus(){
    try{
      ...
      state.referenceReady=!!(s.master&&s.supplier);
      if(state.referenceReady){
        const masterDate=s.master&&s.master.savedAt?new Date(s.master.savedAt).toLocaleDateString():'';
        els.referenceReady.textContent=masterDate?`Ready · cached ${masterDate}`:'Ready';
        els.referenceDot.className='reference-dot ok';
      }else{
        els.referenceReady.textContent='Admin setup required';
        els.referenceDot.className='reference-dot warn';
      }
      renderFiles();
    }catch(err){
      console.error(err);state.referenceReady=false;els.referenceReady.textContent='Unavailable';els.referenceDot.className='reference-dot warn';renderFiles();
    }
  }
  ...
  function renderFiles(){
    ...
    if(!state.referenceReady)setStatus('Reference data is not ready on this computer. An admin can update it from the small Admin link above.','warn');
  ...
  function renderResults(){
    ...
    const warns=[...(r.warnings||[])];if(t.lowConfidenceLines)warns.push(`${t.lowConfidenceLines} matched line(s) have LOW confidence and should be reviewed.`);warns.push('The Excel download keeps every POS order line in the exact uploaded POS order sequence. Invoice data is merged into those rows; unmatched invoice-only lines, if any, are appended after the POS order block.');
  ...
  async function run(){
    if(!state.pos||!state.invoices.length||!state.referenceReady){setStatus('Reference data is not ready on this computer. Ask an admin to update it.','warn');return;}
  ...
  els.downloadBtn.onclick=async()=>{if(!state.result||!state.docs.length||!state.refs)return;const old=els.downloadBtn.textContent;els.downloadBtn.disabled=true;els.downloadBtn.textContent='Building Excel…';try{await PHF.exportReference(state.docs,state.refs,state.posParsed,state.result);}catch(err){console.error(err);setStatus(err&&err.message?err.message:String(err),'warn');}finally{els.downloadBtn.disabled=false;els.downloadBtn.textContent=old;}};
