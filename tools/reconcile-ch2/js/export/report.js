(function(global){
  ...
  function font(color,bold=false){return {name:'Google Sans',size:8,bold,color:{argb:argb(color)}};}
  ...
  async function buildWorkbook(output){
    ...
    ws.getCell(1,4).value=`INVOICE NUMBERS: ${invoiceCount}`;
    ws.getCell(1,5).value=`UNIQUE REFS: ${refCount}`;
    ...
    for(let c=1;c<=H.length;c++){
      ...
      const t=ws.getCell(totalsRow,c);t.fill=fill('D9EAF7');t.font=font('1F2937',true);t.border=BORDER;t.alignment={vertical:'middle',wrapText:true};
    }
    for(let r=dataStart;r<=dataEnd;r++){
      const base=(r%2===0)?'F3F6F9':'FFFFFF';
      for(let c=1;c<=H.length;c++){const cell=ws.getCell(r,c);cell.fill=fill(base);cell.font=font('1F2937',false);cell.border=BORDER;cell.alignment={vertical:'middle',wrapText:true};}
    ...
    // Normalize the whole output range to Excel 'Middle Align' vertically.
