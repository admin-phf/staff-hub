(function(global){
  ...
  const DB_NAME='PHFStaffHub';
  const DB_VERSION=1;
  const STORE='referenceFiles';
  ...
  function percent(v){
    let n=num(v); if(n==null) return null; if(n>0 && n<=1) n*=100; return n;
  }
  ...
  function findHeader(matrix,requirements,maxRows=100){
    let best=null;
    for(let r=0;r<Math.min(matrix.length,maxRows);r++){
      const map=headerMap(matrix[r]); let score=0, ok=true;
      for(const req of requirements){const found=aliasCol(map,req)>=0;if(found)score+=10;else ok=false;}
      if(ok && (!best||score>best.score))best={row:r,map,score};
    }
    return best;
  }
  async function readWorkbook(blob){
    if(!global.XLSX) throw new Error('Spreadsheet reader did not load. Refresh the page and try again.');
    const ab=await blob.arrayBuffer();
    return global.XLSX.read(ab,{type:'array',raw:true,cellDates:true});
  }
  ...
  const MASTER_ALIASES={
    ...
    supplier:['POS_SUPPLIER','POS SUPPLIER','SUPPLIER'],
    supplierName:['POS_SUPPLIER_NAME','POS SUPPLIER NAME','SUPPLIER_NAME','SUPPLIER NAME'],
    supplierNumber:['POS_SUPPLIER_NUMBER','POS SUPPLIER NUMBER','SUPPLIER_NUMBER','SUPPLIER NUMBER'],
    barcode:['POS_MASTER_BARCODE','POS MASTER BARCODE','MASTER_BARCODE','MASTER BARCODE','POS_MAIN_ID','POS MAIN ID','BARCODE'],
    plu:['POS_PLU','POS PLU','PLU'],
    brand:['POS_BRAND','POS BRAND','MASTER_BRAND','MASTER BRAND','BRAND'],
    descr:['POS_DESCR','POS DESCR','POS_DESCRIPTION','POS DESCRIPTION','DESCRIPTION','DESCR'],
    wsp:['POS_WSP_EXCGST','POS WSP EXCGST','WSP_EXCGST','WSP EXCGST','WHOLESALE EX GST'],
    ch2Wholesale:['CH2_WHOLESALE_EX_GST','CH2 WHOLESALE EX GST','CH2_WHOLESALE','CH2 WHOLESALE','PGC_WHOLESALE_EX_GST','PGC WHOLESALE EX GST','WHOLESALE EX GST','RAW WHOLESALE','CH2 RAW WHOLESALE'],
    last:['POS_LAST_PRICE','POS LAST PRICE','LAST_PRICE','LAST PRICE'],
    gst:['POS_GST_TAX_PC','POS GST TAX PC','GST_TAX_PC','GST TAX PC','GST_PC','GST PC','GST %'],
    rrp:['POS_RRP_INCGST','POS RRP INCGST','POS_RRP','POS RRP','RRP_INCGST','RRP INCGST','CURRENT RRP','RRP']
  };
  ...
  async function parsePosMaster(blob){
    ...
    for(const name of wb.SheetNames){
      const matrix=matrixFor(wb,name); const h=findHeader(matrix,[MASTER_ALIASES.code],100);
      if(!h) continue;
      const core=['barcode','plu','brand','descr','last','rrp'].reduce((n,k)=>n+(aliasCol(h.map,MASTER_ALIASES[k])>=0?1:0),0);
      const score=100+core*10;
      if(!chosen||score>chosen.score)chosen={name,matrix,h,score};
    }
    ...
    const byCode=new Map(), byBarcode=new Map(); let duplicates=0;
    for(let r=chosen.h.row+1;r<chosen.matrix.length;r++){
      const row=chosen.matrix[r]||[]; const code=digits(valueAt(row,cols.code)); if(!code) continue;
      const record={
        MASTER_CODE:code,
        POS_SUPPLIER_RAW:clean(valueAt(row,cols.supplier)),
        POS_SUPPLIER_NAME:clean(valueAt(row,cols.supplierName)),
        POS_SUPPLIER_NUMBER:digits(valueAt(row,cols.supplierNumber)),
        POS_MASTER_BARCODE:barcode(valueAt(row,cols.barcode)),
        POS_PLU:clean(valueAt(row,cols.plu)).replace(/\.0+$/,''),
        POS_BRAND:clean(valueAt(row,cols.brand)),
        POS_DESCR:clean(valueAt(row,cols.descr)),
        POS_WSP_EXCGST:num(valueAt(row,cols.wsp)),
        POS_CH2_WHOLESALE_EX_GST:num(valueAt(row,cols.ch2Wholesale)),
        POS_LAST_PRICE:num(valueAt(row,cols.last)),
        POS_GST_TAX_PC:num(valueAt(row,cols.gst)),
        POS_RRP_INCGST:num(valueAt(row,cols.rrp))
      };
      ...
      const bc=record.POS_MASTER_BARCODE;if(bc && !byBarcode.has(bc))byBarcode.set(bc,record);
    }
    const fuzzy=[]; for(const rec of byCode.values()) if(clean(rec.POS_DESCR))fuzzy.push(rec);
    return {byCode,byBarcode,fuzzy,info:{sheet:chosen.name,headerRow:chosen.h.row+1,records:byCode.size,duplicates}};
  }

  function locateSheet(wb,requirements,preferredNames=[]){
    const ordered=[...preferredNames.filter(n=>wb.SheetNames.includes(n)),...wb.SheetNames.filter(n=>!preferredNames.includes(n))];
    for(const name of ordered){const matrix=matrixFor(wb,name);const h=findHeader(matrix,requirements,25);if(h)return {name,matrix,h};}
    return null;
  }
  async function parseSupplierMerge(blob){
    const wb=await readWorkbook(blob);
    const supplierMap=new Map(), ch2SupplierLookup=new Map(), discountRules=[], brandMap=new Map();

    let s=locateSheet(wb,[['POS ACCNO','POS_ACCNO','ACCNO'],['POS ACNAME','POS_ACNAME','ACNAME']],['SRC_POS_SUPPLIERS']);
    if(s){const cAcc=aliasCol(s.h.map,['POS ACCNO','POS_ACCNO','ACCNO']), cName=aliasCol(s.h.map,['POS ACNAME','POS_ACNAME','ACNAME']);
      for(let r=s.h.row+1;r<s.matrix.length;r++){const row=s.matrix[r]||[];const acc=digits(valueAt(row,cAcc)), name=clean(valueAt(row,cName));if(acc&&name&&!supplierMap.has(acc))supplierMap.set(acc,name);}}
    ...
    if(s){
      const cSub=aliasCol(s.h.map,['SUP SUB ID','SUP_SUB_ID']);
      const cNo=aliasCol(s.h.map,['POS SUPPLIER NUMBER','POS_SUPPLIER_NUMBER','POS SUPPLIER']);
      const cName=aliasCol(s.h.map,['POS SUPPLIER NAME','POS_SUPPLIER_NAME']);
      const cBrand=aliasCol(s.h.map,['SUP BRAND','SUP_BRAND']);
      const cProduct=aliasCol(s.h.map,['SUP PRODUCT','SUP_PRODUCT']);
      const cBc=aliasCol(s.h.map,['SUP BARCODE','SUP_BARCODE','BARCODE']);
      for(let r=s.h.row+1;r<s.matrix.length;r++){const row=s.matrix[r]||[];const code=digits(valueAt(row,cSub));if(!code||ch2SupplierLookup.has(code))continue;ch2SupplierLookup.set(code,{POS_SUPPLIER_NUMBER:digits(valueAt(row,cNo)),POS_SUPPLIER_NAME:clean(valueAt(row,cName)),SUP_BRAND:clean(valueAt(row,cBrand)),SUP_PRODUCT:clean(valueAt(row,cProduct)),SUP_BARCODE:barcode(valueAt(row,cBc))});}
    }
    ...
    if(s){const cSup=aliasCol(s.h.map,['SUP BRAND','SUP_BRAND']), cPos=aliasCol(s.h.map,['POS BRAND','POS_BRAND']);for(let r=s.h.row+1;r<s.matrix.length;r++){const row=s.matrix[r]||[];const sup=normHeader(valueAt(row,cSup)), pos=normHeader(valueAt(row,cPos));if(sup&&pos&&!brandMap.has(sup))brandMap.set(sup,pos);}}
  ...
  async function load(kind){const db=await openDb();return new Promise((resolve,reject)=>{const tx=db.transaction(STORE,'readonly');const req=tx.objectStore(STORE).get(kind);req.onsuccess=()=>resolve(req.result||null);req.onerror=()=>reject(req.error);});}
  ...
  async function parseStored(){
    ...
    if(!st.master) throw new Error('POS/master reference data is not loaded on this computer. Open Reference data and load the latest merged_alligned_pos_supplier_uhp_full workbook.');
    if(!st.supplier) throw new Error('Supplier/discount reference data is not loaded on this computer. Open Reference data and load the latest POS DB & SUPPLIER MERGE workbook.');
    const [master,supplier]=await Promise.all([parsePosMaster(st.master.blob),parseSupplierMerge(st.supplier.blob)]);
    return {master,supplier,meta:st};
  }
